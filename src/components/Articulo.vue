<template>
    <div class="articulo">
        <div class="product-grid">
            <div class="product-image" style="height: 300px; display: flex; align-items: center; justify-content: center">
                <a :href="'../preview/'+ articulo.codigo_articulo">
                    <img class="pic-1" :src="require('../assets/' + articulo.referencia + '.jpeg')" style="width: 100%; height:100%">
                    <img class="pic-2" :src="require('../assets/' + articulo.referencia + '.jpeg')" style="width: 120%; height:120%">
                </a>
                <ul class="social">
                    <li><a :href="'../preview/'+ articulo.codigo_articulo" data-tip="Vista Previa"><i class="fa fa-eye"></i></a></li>
                    <li><a data-tip="Añadir a Favoritos" v-on:click="addFavorites"><i class="fa fa-heart"></i></a></li>
                    <li><a data-tip="Añadir al Carrito" v-on:click="addCar"><i class="fa fa-shopping-cart"></i></a></li>
                </ul>
                <span class="product-new-label">{{ articulo.nombre_estado }}</span>
                <span class="product-discount-label">{{ articulo.descuento }}%</span>
            </div>
            <ul class="rating" style="display: flex; justify-content: center">
                <li style="align-itmes: row" v-for="i in 5" :key="i">
                    <span class="fa fa-star" v-if="i <= articulo.calificacion"></span>
                    <span class="fa fa-star disable" v-else></span>
                </li> 
            </ul>
            <div class="product-content">
                <h3 class="title" style="margin-bottom: 0px"><a :href="'../preview/'+ articulo.codigo_articulo">{{ articulo.nombre_articulo }}</a></h3>
                <div class="price" style="margin-bottom: 20px">${{ articulo.valor_final }}
                    <span>${{ articulo.precio_venta }}</span>
                </div>
            </div>
        </div>
    </div>
        ...
        methods:{
            addCar(){
                if(this.$store.state.usuario){
                    ...
                    axios.post('/shoppingcart/add', {codigo_articulo: this.articulo.codigo_articulo, id_cliente: this.$store.state.usuario.id }).then(res => {
            ...
            addFavorites(){
                if(this.$store.state.usuario){
                    this.$store.dispatch("generarState");
                    axios.post('/favorites/add', {codigo_articulo: this.articulo.codigo_articulo, id_cliente: this.$store.state.usuario.id }).then(res => {
